 ...
 */
fun initKoin(apiToken: String, database: BookmarkDatabase): KoinApplication {
    val app = startKoin {
        modules(
...
fun iosNetworkModule(apiToken: String) = module {
    single<HttpClient> {
        HttpClient {
...
fun iosDatabaseModule(database: BookmarkDatabase) = module {
...
class CloseableJob(private val job: Job) {
    fun cancel() {
        job.cancel()
    }
}
